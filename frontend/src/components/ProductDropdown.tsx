 ...
 *
 * Visual only. No routing, no click handlers, no state.
 * Plug into Navbar.tsx by wrapping the "Product" link in a
 * `group relative` div and rendering <ProductDropdown /> below it.
 * The panel uses `invisible opacity-0 group-hover:visible group-hover:opacity-100`
 * so it appears on hover with zero JS.
 */
...
const FEATURES = [
  ...
  },
  {
    ...
    desc: 'Copilot for your product team',
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
  ...
  },
  {
    ...
    desc: 'Connect to your IDE and AI tools',
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
  ...
  return (
    ...
    ">
      ...
      <div className="flex divide-x divide-white/[0.06]">
          ...
          <div className="space-y-0.5">
            {FEATURES.map(({ label, desc, icon }) => (
              <div
                key={label}
                className="
                  group/item flex cursor-default items-start gap-3
        ...
        <div className="w-[185px] flex-shrink-0 p-5">
          <p className="mb-3 px-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-zinc-500">
            Use Cases
          </p>
          ...
          <div className="space-y-0.5">
            {USE_CASES.map(({ label, icon }) => (
              <div
                key={label}
                className="
                  group/case flex cursor-default items-center gap-2.5
                  rounded-xl px-3 py-2.5
                  transition-colors duration-150
                  hover:bg-white/5
                "
      ...
      <div className="flex items-center gap-6 border-t border-white/[0.06] bg-zinc-950/60 px-6 py-3">
        <a href="#" className="text-xs font-medium text-zinc-500 transition-colors hover:text-indigo-400">
          Watch demo →
        </a>
        <a href="#" className="text-xs font-medium text-zinc-500 transition-colors hover:text-indigo-400">
          Contact enterprise →
