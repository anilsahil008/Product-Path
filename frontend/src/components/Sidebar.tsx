          ...
          {sessions.length === 0 ? (
            ...
            sessions.map(session => (
              <div key={session.id} className="group relative">
                ...
                {/* 3-dot hover action — UI only */}
                <button className="absolute right-1.5 top-1/2 -translate-y-1/2 hidden rounded-md p-1 text-zinc-500 hover:bg-zinc-700 hover:text-zinc-300 group-hover:flex transition-colors">
                  {icons.dots}
                </button>
              </div>
            ))
